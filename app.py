...
# =====================================================
def get_nearby_therapists(city, limit=5):
    try:
        conn = get_db()
        cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("""
            SELECT name, latitude, longitude, contact
            FROM therapists
            WHERE city = %s
            LIMIT %s
        """, (city, limit))
...
def chat():
    try:
        ...
        # Recommendations only for non-happy, non-normal low/mid/high risk
        recommendations = []
        ...
        # Get therapists list only if critical
        therapists = []
